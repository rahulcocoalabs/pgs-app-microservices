...
exports.getTutorAppointmentRequestList1 = async (req, res) => {
  ...
  var list = await classRequest.find({status:1,tutorId:userId},{},pageParams).populate([{path:'userId',select:{'image':1,'firstName':1}},{path:'classId',select:{'title':1}}]).catch(err=>{
    return {
      success:0,
      message:err.message
    }
